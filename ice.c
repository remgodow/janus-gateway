...
void janus_ice_relay_rtcp_internal(janus_handle *handle, janus_plugin_rtcp *packet, gboolean filter_rtcp);
	...
	if(janus_is_rtp(buf, len)) {
		...
		if(janus_is_webrtc_encryption_enabled() && (!pc->dtls || !pc->dtls->srtp_valid || !pc->dtls->srtp_in)) {
			...
			if(res != srtp_err_status_ok) {
				...
				janus_plugin_rtp rtp = { .video = video, .mindex = medium->mindex, .buffer = buf, .length = buflen };
				...
				*header = backup;
				...
				if(nacks_count) {
					...
					if(res > 0) {
                        ...
                        janus_plugin_rtcp rtcp = { .video = video, .mindex = medium->mindex, .buffer = nackbuf, .length = res };
		...
		if(janus_is_webrtc_encryption_enabled() && (!pc->dtls || !pc->dtls->srtp_valid || !pc->dtls->srtp_in)) {
			...
			if(res != srtp_err_status_ok) {
				...
				janus_plugin_rtcp rtcp = { .video = video, .mindex = medium->mindex, .buffer = buf, .length = buflen };
				janus_plugin *plugin = (janus_plugin *)handle->app;
...
static gboolean janus_ice_outgoing_transport_wide_cc_feedback(gpointer user_data) {
	...
	if(pc && pc->do_transport_wide_cc && medium) {
		...
		while((packets_len = g_queue_get_length(packets)) > 0) {
			...
			janus_plugin_rtcp rtcp = { .video = TRUE, .mindex = medium->mindex, .buffer = rtcpbuf, .length = len };
			janus_ice_relay_rtcp_internal(handle, &rtcp, FALSE);
...
static gboolean janus_ice_outgoing_rtcp_handle(gpointer user_data) {
	...
	for(mi=0; mi<g_hash_table_size(pc->media); mi++) {
		...
		if(medium->out_stats.info[0].packets > 0) {
			...
			/* Enqueue it, we'll send it later */
            		janus_plugin_rtcp rtcp = { .video = TRUE, .mindex = medium->mindex, .buffer = rtcpbuf, .length = srlen+sdeslen };
            		janus_ice_relay_rtcp_internal(handle, &rtcp, FALSE);
		...
		if(medium->recv) {
			...
			for(vindex=0; vindex<3; vindex++) {
				if(medium->rtcp_ctx[vindex] && medium->rtcp_ctx[vindex]->rtp_recvd) {
					...
					/* Enqueue it, we'll send it later */
                    janus_plugin_rtcp rtcp = { .video = TRUE, .mindex = medium->mindex, .buffer = rtcpbuf, .length = 32 };
                    janus_ice_relay_rtcp_internal(handle, &rtcp, FALSE);
                    if(vindex == 0)
                    {
                        /* Check if we detected too many losses, and send a slowlink event in case */
...
static gboolean janus_ice_outgoing_traffic_handle(janus_handle *handle, janus_ice_queued_packet *pkt) {
	...
	if(pkt->control) {
		...
		if(pkt->type == JANUS_ICE_PACKET_AUDIO || pkt->type == JANUS_ICE_PACKET_VIDEO) {
			...
			janus_dtls_wrap_sctp_data(pc->dtls, pkt->label, pkt->type == JANUS_ICE_PACKET_TEXT, pkt->data, pkt->length);
#endif
...
void janus_ice_relay_rtp(janus_handle *handle, janus_plugin_rtp *packet) {
	if(!handle || !handle->pc || handle->queued_packets == NULL || packet == NULL || packet->buffer == NULL ||
		...
		return;
	/* Find the right medium instance */
	janus_handle_webrtc_medium *medium = (packet->mindex != -1 ?
		...
		if(medium->ssrc_peer[1]) {
			...
			janus_rtcp_fix_ssrc(NULL, plibuf, sizeof(plibuf), 1,
				medium->ssrc, medium->ssrc_peer[1]);
            		janus_plugin_rtcp rtcp = { .video = TRUE, .mindex = medium->mindex, .buffer = plibuf, .length = sizeof(plibuf) };
            		janus_ice_relay_rtcp_internal(handle, &rtcp, FALSE);
		}
...
void janus_ice_send_pli(janus_handle *handle, int mindex) {
	...
	janus_plugin_rtcp rtcp = { .video = TRUE, .mindex = mindex, .buffer = rtcpbuf, .length = 12 };
...
void janus_ice_send_remb(janus_handle *handle, int mindex, uint32_t bitrate) {
	...
	janus_plugin_rtcp rtcp = { .video = TRUE, .mindex = mindex, .buffer = rtcpbuf, .length = 24 };
	janus_ice_relay_rtcp(handle, &rtcp);
}
