    ...
    while(property != JANUS_SDP_OA_DONE) {
        ...
        if(property == JANUS_SDP_OA_MLINE || property == JANUS_SDP_OA_DONE) {
            ...
            if(mline_enabled) {
                ...
                if(type == JANUS_SDP_AUDIO) {
                    if(janus_sdp_generate_offer_mline(offer,
                                                      ...
                                                      JANUS_SDP_OA_EXTENSION, extmaps, extids,
                                                      JANUS_SDP_OA_DONE
                    ) < 0) {
                        janus_sdp_destroy(offer);
                        va_end(args);
            ...
            if(extmap != NULL && id > 0 && id < 15) {
                ...
                char *new_extmap = g_hash_table_lookup(extids, GINT_TO_POINTER(id));
                if((property == JANUS_SDP_OA_EXTENSION && new_extmap != NULL && strcasecmp(new_extmap, extmap))) {
...
int janus_sdp_generate_offer_mline(janus_sdp *offer, ...) {
    ...
    while(property != JANUS_SDP_OA_DONE) {
        if(property == JANUS_SDP_OA_DIRECTION) {
            ...
            extmaps = va_arg(args, GHashTable*);
    ...
    if(type == JANUS_SDP_AUDIO || type == JANUS_SDP_VIDEO) {
        ...
        /* Check if we need to add media extensions to the SDP */
        if(extids != NULL) {
