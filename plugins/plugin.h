...
struct janus_callbacks {
	 ...
	 * @param[in] handle The plugin/gateway session that will be used for this peer */
	void (* const send_pli)(janus_plugin_session *handle, int mindex);
	/*! \brief Helper to ask for a keyframe via a RTCP PLI
	 * @note This is a shortcut, as it is also possible to do the same by crafting
	...
	void (* const send_remb)(janus_plugin_session *handle, int mindex, guint32 bitrate);
...
struct janus_plugin_rtp {
	...
	gboolean video;
    /*! \brief Index of the target stream (relative to the SDP), -1 for the first audio/video stream */
    int mindex;
