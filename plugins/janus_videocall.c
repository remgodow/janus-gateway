...
void janus_videocall_incoming_rtcp(janus_plugin_session *handle, janus_plugin_rtcp *packet) {
	...
	if(gateway) {
		...
		if(bitrate > 0) {
			...
			/* No limit ~= 10000000 */
			gateway->send_remb(handle, -1, session->bitrate ? session->bitrate : 10000000);
			return;
		}
	...
	while(g_atomic_int_get(&initialized) && !g_atomic_int_get(&stopping)) {
		...
		if(!strcasecmp(request_text, "list")) {
			...
			if(bitrate) {
				...
				gateway->send_remb(session->handle, -1, session->bitrate ? session->bitrate : 10000000);
			}
			janus_videocall_session *peer = session->peer;
			if(substream) {
				...
				if(session->sim_context.substream_target == session->sim_context.substream) {
					...
					if(peer && peer->handle)
						gateway->send_pli(peer->handle, -1);
				}
			}
			if(temporal) {
